...
# Восемь румбов, а не «север/юг»: азимут в настройках произвольный, и 45°
# честнее показать как ↗, чем округлить до севера.
_ROSE_A = "↑↗→↘↓↙←↖"


def ant_mark(nid):
    """Стрелка антенны рядом с именем ноды: румб по азимуту из ⚙-настроек карты,
    ◎ у круговой, пусто — если про антенну ничего не заявлено.

    Расшифровку в эфир НЕ шлём: каждый лишний байт снижает шанс доставки
    broadcast'а, а объяснять символы дешевле один раз в канале, чем в каждом
    ответе."""
    ...
    if g.get("ant") == "dir" and g.get("dir") is not None:
        try:
            return _ROSE_A[int(round(float(g["dir"]) % 360 / 45)) % 8]
...
def gh_watch_loop():
    ...
    Формулировка «обновление», а не «ответ»: фид поднимает запись и на новый
    комментарий, и на правку текста автором — различить их без токена нельзя,
    поэтому обещать «ответ» нечестно."""
    seen = {}
    try:
