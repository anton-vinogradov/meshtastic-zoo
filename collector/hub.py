...
# Румбы для метки антенны: 8 стрелок по 45°, начиная с севера. Компас, а не
# «север/юг»: азимут в конфиге произвольный, и 45° честнее округлять до ↗.
_ROSE = "↑↗→↘↓↙←↖"


def ant_mark(nid):
    """Символ антенны своей ноды для эфирных ответов: стрелка по азимуту из
    ⚙-настроек карты, ◎ у круговой, пусто — если про ноду ничего не заявлено.
    Пингующему это отвечает на «а почему одна слышит, а вторая нет»."""
    g = (CFG.get("geo") or {}).get(nid)
    if not isinstance(g, dict):
        return ""
    if g.get("ant") == "dir" and g.get("dir") is not None:
        try:
            return _ROSE[int(round(float(g["dir"]) % 360 / 45)) % 8]
...
def ping_reply(pid, frm, frm_name):
    ...
    nm_of = lambda i: ((CFG.get("names") or {}).get(i, i[-4:])) + ant_mark(i)
    direct, relayed = [], {}
    for nid, v in order:
