...
# Румбы СЛОВАМИ: стрелка ↑ рядом с именем читалась как «уровень сигнала» —
# символ понятен только тому, кто уже знает про наши антенны. В эфир пишем то,
# что человек поймёт без легенды. Восемь румбов, а не «север/юг»: азимут
# произвольный, и 45° честнее назвать северо-востоком, чем севером.
_ROSE_W = ("север", "северо-восток", "восток", "юго-восток",
           "юг", "юго-запад", "запад", "северо-запад")


def ant_word(nid):
    """Куда смотрит антенна своей ноды — словом. Берётся из ⚙-настроек карты
    (там азимут и так задаётся при размещении). None — если про антенну ничего
    не заявлено: выдумывать «кругом» за молчащий конфиг не будем."""
    g = (CFG.get("geo") or {}).get(nid)
    if not isinstance(g, dict) or not g.get("ant"):
        return None
    if g.get("ant") == "dir" and g.get("dir") is not None:
        try:
            return _ROSE_W[int(round(float(g["dir"]) % 360 / 45)) % 8]
...
def ping_reply(pid, frm, frm_name):
    ...
    txt = "🏓 " + (pre + "\n" if pre else "") + "\n".join(bits)
    # КУДА СМОТРЯТ АНТЕННЫ — последней строкой, сгруппировав ноды по румбу:
    # «FCB слышит, FCA нет» без этого выглядит случайностью, хотя это ровно про
    # сектор. Строка НЕОБЯЗАТЕЛЬНАЯ: лимит эфира 200 байт, и если с ней не
    # влезаем (длинный префикс, много нод), лучше отдать ответ без неё, чем
    # обрезать посреди слова.
    grp = {}
    for nid, _v in order:
        w = ant_word(nid)
        if w:
            grp.setdefault(w, []).append(nm_of(nid))
    if grp:
        seq = [w for w in _ROSE_W + ("кругом",) if w in grp]
        line = "🧭 " + " · ".join(f"{w} {','.join(grp[w])}" for w in seq)
        if len(f"{txt}\n{line}".encode()) <= 200:
            txt = f"{txt}\n{line}"
