...
- **Серые квадратики с числом хопов** — нода, которая была прямым
  ...
  Такая нода помнится по *времени последнего прямого приёма*, поэтому
  один пропущенный скан её не теряет; когда этот последний 0-хоп уходит
  за окно памяти — нода забывается.
...
таймауты подключения/опроса, `hopMemoryH` (сколько часов держать
бывшего прямого соседа на карте серой многохоповой нодой после
последнего прямого приёма — по умолчанию 24) и `known` / `names` —
запасные карты IP↔radio-id и имён на случай, если нода не отвечает.
