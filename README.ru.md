...
Если кто-то пишет в канал ровно `ping` (или `пинг`), хаб отвечает цитатой:
...
в живой разговор. Ограничители: пауза `pingCooldownS` (600 с) на одного
...
иначе два хаба перепингуют друг друга. Выключается `"pingReply": false`
в `collector/config.json`.
