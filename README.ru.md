...
![Карта связности: свои ноды окрашены по площадкам, соседи услышаны по радио, стрелки — по качеству линка, на каждой подпись SNR.](docs/screenshot.png)

*(Адреса своих нод и имена соседей на кадре скрыты.)*
...
- **Серые квадратики** — бывший прямой сосед (0 хопов), которого больше
  не слышно напрямую. Прямые соседи постоянно скачут 0↔несколько хопов
  (норма РЧ), поэтому нода становится серой, только если прямого приёма
  нет уже несколько минут подряд — мгновенный флап не в счёт. Серой её
  держат две причины: либо её теперь **достаёт через ретрансляторы** (на
  плече вместо SNR — число хопов `1 hop`, `3 hops`…, вплоть до
  `hopMaxShow`), либо она **замолчала** (сейчас её не слышит никто), но
  была прямой в пределах последнего часа — тогда она остаётся на своём
  последнем месте, чтобы нода, с которой ты только что общался, не
  исчезала бесследно. В любом случае — серая с пунктирной рамкой, держится
  до часа без прямого приёма, потом забывается. Галочка **former
  neighbor** («бывший сосед») в легенде прячет/показывает эти серые ноды
