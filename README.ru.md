...
Вопросы, идеи или карта своего зоопарка —
в [Обсуждения](https://github.com/anton-vinogradov/meshtastic-zoo/discussions).
...
### Автоответ на слова-триггеры

Если кто-то пишет в канал ровно одно из слов-триггеров (по умолчанию
`ping`, `пинг`, `test`, `тест`, `проверка`, `hi`, `привет` — список
правится в ⚙, там же тумблер включения), хаб отвечает цитатой:
какие твои ноды его услышали и как далеко —
`🏓 напрямую: FCA +9.2, FC1 −7.5 · через 3🐇: FCB`. Отвечает нода,
услышавшая лучше всех: её вероятнее услышат в ответ.

SNR показывается **только для прямого приёма**. У пакета, дошедшего через
ретрансляторы, SNR описывает передатчик последнего реле, а не отправителя,
поэтому такие приёмы сворачиваются в число хопов: если напрямую никто не
слышал, ответ честно скажет `через 4🐇: … · напрямую не слышим`.

Эфира на это не тратится: приёмы того же пакета уже собраны, ответ —
один broadcast. Слово должно быть сообщением целиком, иначе бот влезал бы
в живой разговор («привет» ответ получит, «привет всем» — нет); регистр
и знаки вокруг не важны. Ограничители: пауза `pingCooldownS` (600 с) на одного
отправителя, `pingGapS` (60 с) на канал в целом, молчание при загрузке
канала выше `busyChUtil`, и никогда не отвечаем на ping своих же нод —
иначе два хаба перепингуют друг друга. Выключается тумблером в ⚙ (или
`"pingReply": false` в `collector/config.json`).
