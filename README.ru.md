...
- **Серые квадратики с числом хопов** — нода, которая была прямым
  соседом (0 хопов), а теперь достаёт до тебя только через ретрансляторы.
  Она остаётся на своём месте, но рисуется серой с пунктирной рамкой, а
  на плече вместо SNR стоит число хопов (`1 hop`, `2 hops`…) — прямого
  линка для замера уже нет. Чем больше хопов, тем дальше она отъезжает.
  Считаются только близкие ретрансляции — по умолчанию 1–2 хопа; нода,
  скакнувшая на 3+ хопа сразу после прямой связи, — это обычно разброс
  маршрутов меша, а не переезд. Такая нода помнится несколько минут после
  того, как последний раз была на карте, поэтому один пропущенный скан её
  не теряет; когда её нет дольше этого окна — забывается.
...
маршрутизации, а не переездом — по умолчанию 2), `hopMemoryMin` (сколько
минут держать бывшего соседа на карте после последнего появления —
по умолчанию 5) и `known` / `names` — запасные карты IP↔radio-id и имён
на случай, если нода не отвечает.
